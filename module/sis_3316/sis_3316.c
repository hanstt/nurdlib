	...
	for (i = 0; i < N_CHANNELS; ++i) {
		...
		if ((i % N_CH_PER_ADC) == 0) {
			LOGF(verbose)(LOGL, "ADC %u: range = %d -> "
			    ...
			    a_module->config.range[adc_i], counts_per_V);
		}
		a_module->config.threshold[i] = 0x08000000 +
		    (uint32_t)(counts_per_V
			           * a_module->config.threshold_V[i]
			           * (double)a_module->config.peak[i]);
		a_module->config.threshold_high_e[i] = 0x08000000 +
		    (uint32_t)(counts_per_V
			           * a_module->config.threshold_high_e_V[i]
			           * (double)a_module->config.peak[i]);
		LOGF(verbose)(LOGL, "threshold_low[%d] = %f V -> 0x%08x, "
		    "threshold_high[%d] = %f V -> 0x%08x",
		    i, a_module->config.threshold_V[i],
		    a_module->config.threshold[i],
		    i, a_module->config.threshold_high_e_V[i],
		    a_module->config.threshold_high_e[i]);
	}
